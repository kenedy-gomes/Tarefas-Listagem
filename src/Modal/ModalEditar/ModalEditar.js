...
function ModalEditar({ item: { _id, name, description, status } }) {
  const baseURL3 = "http://localhost:3001/tarefas";

  const handleSubmit1 = (data) => {
    axios
      .put(baseURL3 + "/" + data._id, {
        name: data.name,
  ...
  return (
    ...
    >
      {({ values, handleChange, handleSubmit, setFieldValue }) => (
        <>
          <a onClick={onOpen}>
            <EditIcon float={right} cursor={"pointer"} />
          </a>
          <Modal
          ...
          >
            <ModalOverlay />
            <ModalContent justifyContent={"center"} margin={"10em 0em 0em 0em"}>
